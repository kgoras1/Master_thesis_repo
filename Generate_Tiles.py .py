...
def generate_deepzoom_tiles(svs_file, output_dir, tile_size=256, overlap=0):
    """
    ...
    for level in range(level_count):
        ...
        for tile_x in range(num_tiles_x):
            for tile_y in range(num_tiles_y):
                ...
                if tile_np.mean() < 230 and avgBkg <= 0.25:
...
output_dir = os.path.join(os.path.dirname(input_dir), "LSCC_3_256nobgr")

# Call the function with user-provided input
generate_deepzoom_tiles_from_path(input_dir, output_dir)


# /Volumes/Seagate Expansion Drive/LSCC/LSCC_3.svs
